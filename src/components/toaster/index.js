...
import Icon_error from '../icons/error';
import Icon_info from '../icons/info';
...
class Toaster extends React.Component {
  ...
  constructor(props) {
    ...
    this.clear_toasts = this.clear_toasts.bind(this);
  ...
  componentDidUpdate() {
    ...
    // Check if there is any new toast compare to locals
    for (let i = 0; i < toasts_global.length; i++) {
      let exists = false;
      ...
      if (exists === false) {
        toasts_new.push({ ...toasts_global[i], animation: true });
      }
    }
    ...
    for (let i = 0; i < toasts_final.length; i++) {
      ...
      icon_img.classList.add(style['toaster-toast-icon']);

      const text_div = document.createElement('div');
      text_div.classList.add(style['toaster-toast-message']);
      ...
      toast_div.appendChild(icon_img);
      toast_div.appendChild(text_div);
